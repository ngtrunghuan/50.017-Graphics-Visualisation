...
public:
	...
	void drawJoints( );
    void traverseJoints(Joint *parent, MatrixStack stack);

	// 1.2. Implement this method a recursive helper to draw a box between each pair of joints
	void drawSkeleton( );
    void traverseBones(Joint *parent, MatrixStack stack);
    ...
    void traverseBindWorldToJoint(Joint *joint);

	// 2.3.2. Implement this method to compute a per-joint transform from
	// joint space to world space in the CURRENT POSE.
	void updateCurrentJointToWorldTransforms();
    void traverseJointToWorld(Joint *joint);
